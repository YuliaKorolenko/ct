		...
		},
		{
			...
			args: args{
				v: struct {
					...
					InInt:  25,
					InNeg:  []int{-25, -22, -30},
					InStr:  []string{"bar", "foo", "bar", "bar"},
					MinInt: []int{11, 12, 13, 14, 15},
				},
			},
			wantErr: false,
		},
		{
			name: "wrong test with slices",
			args: args{
				v: struct {
					OneLower   []string `validate:"len:24"`
					HighNormal []string `validate:"len:5"`
					MaxInt     []int    `validate:"max:20"`
					InInt      []int    `validate:"in:20,25,30"`
				}{
					OneLower:   []string{"abcdef", "dhfjdkdskdj", "sadjsjdj"},
					HighNormal: []string{"abcdef", "aaaaa"},
					MaxInt:     []int{11, 12, 13, 14, 15},
					InInt:      []int{10, 20, 25, 30},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
