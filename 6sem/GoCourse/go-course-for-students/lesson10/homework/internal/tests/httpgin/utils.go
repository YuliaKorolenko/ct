...
func (tc *testClient) testPanic() (int, error) {
	req, err := http.NewRequest(http.MethodGet, tc.baseURL+"/api/v1/ads/panic", nil)
	if err != nil {
		return http.StatusBadRequest, fmt.Errorf("unable to create request: %w", err)
	}

	resp, err := tc.client.Do(req)
	if err != nil {
		return http.StatusBadRequest, fmt.Errorf("unexpected error: %w", err)
	}
