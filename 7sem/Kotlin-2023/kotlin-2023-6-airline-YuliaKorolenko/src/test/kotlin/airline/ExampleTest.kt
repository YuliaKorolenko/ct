...
class ExampleTest {
    ...
    fun testBooking() {
        ...
        testAndCancel {
            ...
            checkEmails("test@example.com", flightId1, "2B", "There's been a mistake")
            Assertions.assertEquals(setOf("1A", "1B", "2A"), booking.freeSeats(flightId1, flightTime))
            ...
            checkEmails("kbats@itmo.ru", flightId1, "There's been a mistake")
            Assertions.assertEquals(setOf("1A", "1B", "2A"), booking.freeSeats(flightId1, flightTime))
            ...
            sleep()
            checkEmails("test@hello.com", "There's been a mistake")
            ...
            sleep()
            checkEmails("ipetrov@mail.ru", "There's been a mistake")
    ...
    fun testCheckIn() {
        ...
        val airlineApplication = AirlineApplication(config, emailService)
        val plane1 = Plane("C", setOf("1A", "1B", "2A", "2B", "3A", "3B", "4A", "5A"))
        val flightId1 = "1"
        val flightTime1 = Clock.System.now() + 1.hours
        ...
        testAndCancel {
            ...
            Assertions.assertEquals(1, display.value.departing.size)
            Assertions.assertEquals(flightId1, display.value.departing[0].flightId)
            Assertions.assertEquals(flightTime1, display.value.departing[0].actualDepartureTime)

            booking.buyTicket(flightId1, flightTime1, "2A", "1", "Ivan Petrov", "ipetrov@mail.ru")
            sleep()

            checkEmails("ipetrov@mail.ru", "successfully bought")

            val flightTime2 = flightTime1 + 1.hours
            management.setCheckInNumber(flightId1, flightTime2, "check")

            Assertions.assertEquals(1, display.value.departing.size)
            Assertions.assertEquals(flightId1, display.value.departing[0].flightId)
            Assertions.assertEquals(flightTime1, display.value.departing[0].actualDepartureTime)
            Assertions.assertNotEquals("check", display.value.departing[0].checkInNumber)

            management.delayFlight(flightId1, flightTime1, flightTime2)
            sleep()

            Assertions.assertEquals(1, display.value.departing.size)
            Assertions.assertEquals(flightId1, display.value.departing[0].flightId)
            Assertions.assertEquals(flightTime2, display.value.departing[0].actualDepartureTime)

            checkEmails("ipetrov@mail.ru", "delayed")
    ...
    fun deleteTest() {
        ...
        val plane1 = Plane("A312", setOf("1A", "1B", "2A", "2B"))
        val plane2 = Plane("B737", setOf("1A", "1B", "2A", "2B", "3A", "3B"))
        val flightIdFirst = "21"
        val flightIdSecond = "2"
        val flightTime1 = Clock.System.now() + 200.milliseconds
        val flightTime2 = Clock.System.now() + 3.hours
        ...
        testAndCancel {
            ...
            management.scheduleFlight(flightIdFirst, flightTime1, plane1)
            management.scheduleFlight(flightIdSecond, flightTime2, plane2)
            ...
            booking.buyTicket(
                ...
                "1",
                "Yulia Korolenko",
                "korolenkoyulia@itmo.ru",
            )
            sleep()

            checkEmails("korolenkoyulia@itmo.ru", flightIdSecond, "1A", "successfully bought")
            ...
            checkEmails("korolenkoyulia@itmo.ru", "1A", "Sorry", "mistake")

            management.cancelFlight(flightIdSecond, flightTime2)
            sleep()

            checkEmails("korolenkoyulia@itmo.ru", "cancelled", "2")
            // Check free seats for cancelled flight
            Assertions.assertEquals(booking.freeSeats(flightIdSecond, flightTime2), emptySet<Int>())
            // Check free seats for departured flight
            Assertions.assertEquals(booking.freeSeats(flightIdFirst, flightTime1), emptySet<Int>())
        }

    }

    @Test
    fun unsupportedBought() {
        val config =
            AirlineConfig(
                audioAlertsInterval = 1.seconds,
                displayUpdateInterval = 100.milliseconds,
        ...
        val plane1 = Plane("A312", setOf("1A", "1B", "2A", "2B"))
        val flightIdFirst = "21"
        val flightTime1 = Clock.System.now() + 3.hours
        ...
        testAndCancel {
            ...
            assertContains(booking.flightSchedule, FlightShortInfo(flightIdFirst, flightTime1))

            booking.buyTicket(flightIdFirst, flightTime1, "2B", "1", "Konstantin Bats", "kbats@itmo.ru")
            sleep()

            checkEmails("kbats@itmo.ru", flightIdFirst, "2B", "successfully bought")

            // can't buy with existing passenger id
            booking.buyTicket(flightIdFirst, flightTime1, "1A", "1", "Yulia Korolenko", "ykor@itmo.ru")
            sleep()

            checkEmails("ykor@itmo.ru", flightIdFirst, "1A", "mistake")

            // can't buy for unexisted flight
            booking.buyTicket("1000", flightTime1, "1A", "1", "Yulia Korolenko", "ykor@itmo.ru")
            sleep()

            checkEmails("ykor@itmo.ru", "1000", "1A", "mistake")

            management.cancelFlight(flightIdFirst, flightTime1)
            sleep()

            checkEmails("kbats@itmo.ru", flightIdFirst, "cancelled")

            // can't buy for cancelled flight
            booking.buyTicket(flightIdFirst, flightTime1, "2A", "1", "Yulia Korolenko", "ykor@itmo.ru")
            sleep()

            checkEmails("ykor@itmo.ru", flightIdFirst, "2A", "mistake")
        }
    }
